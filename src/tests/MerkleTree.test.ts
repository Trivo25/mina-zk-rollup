...
describe('MerkleTree.ts', () => {
  ...
  it('Should form a merkle tree correctly', () => {
    ...
    expect(m.validateProof(m.getProof(0), h_A, expectedMerkleRoot)).toEqual(
      true
    );
    expect(m.validateProof(m.getProof(1), h_B, expectedMerkleRoot)).toEqual(
      true
    );
    expect(m.validateProof(m.getProof(2), h_C, expectedMerkleRoot)).toEqual(
      true
    );
    expect(m.validateProof(m.getProof(3), h_D, expectedMerkleRoot)).toEqual(
      true
    );
    expect(m.validateProof(m.getProof(4), h_E, expectedMerkleRoot)).toEqual(
      true
    );
  });
});
