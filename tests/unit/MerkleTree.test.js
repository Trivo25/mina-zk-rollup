...
import { MerkleTree } from '../../src/lib/merkle_proof/MerkleTree';
describe('MerkleTree unit test', () => {
    ...
    it('should construct and proof a merkle tree', () => {
        ...
        assert(actualMerkleRoot === null || actualMerkleRoot === void 0 ? void 0 : actualMerkleRoot.equals(expectedMerkleRoot).toBoolean(), 'Merkle root is not matching expected merkle root!');
        nodeData.forEach((el, index) => {
            assert(MerkleTree.validateProof(merkleTree.getProof(index), Poseidon.hash([el]), actualMerkleRoot === undefined ? Field(0) : actualMerkleRoot), `Merkle path is not valid for index ${index} and element ${el}`);
        });
    });
});
